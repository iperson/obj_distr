#### Currently does not work, I am rewriting for Blender 2.90+ ####
...
def spawn(my_data, next_vert=0, random_spawn=False, cluster_spawn=False):
    ...
    if overlaps(new_item_bvh, my_data.item_bvh_list):
        ...
        bpy.context.scene.collection.objects.link(new_item)
...
class OBJECT_PT_spawn_objects(bpy.types.Panel):
    ...
    bl_idname = 'OBJECT_PT_DistrPanel'
...
def register():
    bpy.utils.register_class(OBJECT_OT_spawn)
    bpy.utils.register_class(OBJECT_OT_cluster_spawn)
    bpy.utils.register_class(OBJECT_OT_bake_tvcol)
    bpy.utils.register_class(OBJECT_PT_spawn_objects)
